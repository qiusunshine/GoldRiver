...
public class fragment_juejin extends Fragment {
    ...
    protected int getLayoutId() {
        return R.layout.fragment_juejin;
    }
    ...
    protected void initView() {
        listView = findView(R.id.juejin_listview);
        requestQueue = Volley.newRequestQueue(getContext());
        footer = LayoutInflater.from(getContext()).inflate(R.layout.loadmorefooter,null);
        swipeRefreshLayout=findView(R.id.juejin_SwipeRefreshLayout);
    ...
    public void getlist(int j, String thisdate, final boolean isrefresh) {
        ...
        if(j==1)
      url="http://timeline-merger-ms.juejin.im/v1/get_entry_by_timeline?category=5562b410e4b00c57d9b94a92&before=&limit=20&token=eyJhY2Nlc3NfdG9rZW4iOiIzdGZVZDgwY3FVd0NDRkEzIiwicmVmcmVzaF90b2tlbiI6IjFDdWZaSjJlRDMxQUxQUjEiLCJ0b2tlbl90eXBlIjoibWFjIiwiZXhwaXJlX2luIjo2MDQ4MDB9&device_id=ba145078-6ca9-4380-ae44-64d58ce52041&src=android";
        else
            url="http://timeline-merger-ms.juejin.im/v1/get_entry_by_timeline?category=5562b410e4b00c57d9b94a92&before="+thisdate+"T04:08:51.739Z&limit=20&token=eyJhY2Nlc3NfdG9rZW4iOiIzdGZVZDgwY3FVd0NDRkEzIiwicmVmcmVzaF90b2tlbiI6IjFDdWZaSjJlRDMxQUxQUjEiLCJ0b2tlbl90eXBlIjoibWFjIiwiZXhwaXJlX2luIjo2MDQ4MDB9&device_id=ba145078-6ca9-4380-ae44-64d58ce52041&src=android";
        final JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET,
            ...
            public void onResponse(JSONObject response) {
                JSONArray data;
                try {
                    data = response.getJSONObject("d").getJSONArray("entrylist");
                    for (int i = 0; i < data.length(); i++) {
                        try {
                            JSONObject jsonObject = new JSONObject();
                            JSONObject netjsonObject = data.getJSONObject(i);
                            jsonObject.put("title", netjsonObject.getString("title"));
                            jsonObject.put("url", netjsonObject.getString("originalUrl"));
                            jsonObject.put("viewsCount",netjsonObject.getString("viewsCount"));
                            jsonObject.put("collectionCount",netjsonObject.getString("collectionCount"));
                            jsonObject.put("hotIndex",netjsonObject.getString("hotIndex"));
                            dataarry.put(jsonObject);
                        } catch (JSONException ignored) {
                        }
                    }
                    if(number==1){
                            juejinadapter = new juejinadapter(getContext(), dataarry);
                            listView.setAdapter(juejinadapter);
                    }
                    TextView load=findView(R.id.loadmre_footer);
                    load.setText("下一页");
